...
const updateCartDetailsInLocalStorage = (cartDetails) => {
    if (!cartDetails) return; // Nếu cartDetails bị null hoặc undefined thì không làm gì

    const currentUser = JSON.parse(localStorage.getItem('user')) || {};
    currentUser.cartDetails = cartDetails;

    console.log("Cập nhật cartDetails vào localStorage:", currentUser);
...
const userSlice = createSlice({
    ...
    reducers: {
        ...
        authSuccess: (state, action) => {
            const oldUser = JSON.parse(localStorage.getItem('user')) || {}; // Lấy dữ liệu cũ
            const newUser = { 
                ...oldUser, // Giữ lại dữ liệu cũ
                ...action.payload, // Cập nhật dữ liệu mới
                cartDetails: action.payload.cartDetails || oldUser.cartDetails || [] // Giữ giỏ hàng cũ nếu không có trong payload
            };
        
            localStorage.setItem('user', JSON.stringify(newUser)); // Lưu lại user mới
            state.currentUser = newUser;
            state.currentRole = newUser.role;
            state.currentToken = newUser.token;
            state.status = 'success';
            state.response = null;
            state.error = null;
            state.isLoggedIn = true;
        
            console.log("User sau khi đăng nhập:", newUser);
        },
